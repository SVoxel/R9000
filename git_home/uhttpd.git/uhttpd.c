					...
					if( (req = uh_http_header_recv(cl)) != NULL )
					{
						...
						remote_addr = sa_straddr(&cl->peeraddr);
							...
							if( (strcmp(httphost, "clients3.google.com") || strncmp(req->url, "/generate_204",13))
								...
								&& strcmp(httphost, "connectivitycheck.gstatic.com")){
...
int internet_check()
{
    ...
    unlink(FWC_PING);
    if(result==1)
        fprintf(stderr, "AUTO FW CHECK: network is ok\n");
    else 
        fprintf(stderr, "AUTO FW CHECK: network is fail and please check you network\n");
    return result;
}
