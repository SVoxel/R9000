...
DEFINE_SPINLOCK(spin_udpct_lock);
EXPORT_SYMBOL(spin_udpct_lock);
#endif

#if defined(CONFIG_NF_NAT_CONENAT_FAST_SEARCH)
struct hlist_nulls_head *conenat_ct_hash;
EXPORT_SYMBOL(conenat_ct_hash);
DEFINE_SPINLOCK(spin_conenatct_lock);
EXPORT_SYMBOL(spin_conenatct_lock);
#endif
#endif
...
#if defined(CONFIG_NF_NAT) || defined(CONFIG_NF_NAT_MODULE)
#if defined(CONFIG_NF_NAT_SINGLE_UDP_CT_FOR_LAN_WAN_TUPLE)
	if (ct->sudp_hashnode.pprev)
		hlist_nulls_del_rcu(&ct->sudp_hashnode);
#endif
 ...
 */
static struct nf_conn *find_lru_low_prio_nat(struct nf_conn_lan *lan)
{
...
static struct nf_conn *ip_ct_handle_nat_full(struct nf_conn *ct)
{
	int count;
	__u32 addr;
	unsigned int hash;
	...
	if (nat_lan_max != NULL && nat_lan_max != lan) {
		...
		if ((nat_lan_max->count - count >= NAT_SESSION_DIFF_NUM)) {
			found_ct = find_lru_low_prio_nat(nat_lan_max);
			if (found_ct)
				return found_ct;
		}
	}
	...
	if ((lan != NULL) && (ct->status & IPS_NAT_STATIC_HIGH_PRIORITY))
		found_ct = find_lru_low_prio_nat(lan);
...
static void ip_ct_lru_low_prio_update(struct nf_conn *ct)
{
	 ...
	 * As in function find_lru_low_prio_nat(), delete form tail of low_prio_lru (low_prio_lru.prev) */
	list_move(&ct->lan_hash, &lan->low_prio_lru);
}

/* nat conntrack management */
static struct nf_conn *nat_conntrack_management(struct nf_conn *ct)
{
	struct nf_conn *found_ct = NULL;
	struct net *net;
	...
	if (ip_ct_nat_lan_session(ct)) {
		if (nf_conntrack_max && 
			...
			found_ct = ip_ct_handle_nat_full(ct);

			if (found_ct == NULL) {
				net_warn_ratelimited("Internet sessions full, dropping packet.\n");
				return ct;
			}
		}
...
/*  Check single UDP conntrack for all LAN-WAN sessions */
static struct nf_conn *sudp_conntrack_management(struct nf_conn *ct)
{
	struct nf_conn *find_ct = NULL, *tmp;
	struct nf_conntrack_tuple *sudp_tuple = NULL, *h;
	struct hlist_nulls_node *n;
	...
	if (ct->status & IPS_SRC_NAT) {
		if (!(ct->status & IPS_DST_NAT)) {
			sudp_tuple = &ct->tuplehash[IP_CT_DIR_ORIGINAL].tuple;
			nat_type = 1;
		}
	...
	/* Just care SNAT(LAN->WAN) or DNAT(WAN->LAN) sessions */
	if (sudp_tuple == NULL)
		return NULL;

	single_udp_hash = single_udp_hash_conntrack(sudp_tuple, nat_filtering_behavior);

	spin_lock_bh(&spin_udpct_lock);
	rcu_read_lock();
	hlist_nulls_for_each_entry_rcu(tmp, n, &single_udp_ct_hash[single_udp_hash], sudp_hashnode) {
		...
		if (single_udp_hash_tuple_cmp(sudp_tuple, h, nat_filtering_behavior))
			find_ct = tmp;

		if (find_ct && unlikely(nf_ct_is_dying(find_ct) ||
		    !atomic_inc_not_zero(&find_ct->ct_general.use)))
			find_ct = NULL;
	...
	hlist_nulls_add_head_rcu(&ct->sudp_hashnode, &single_udp_ct_hash[single_udp_hash]);
	spin_unlock_bh(&spin_udpct_lock);
...
__nf_conntrack_confirm(struct sk_buff *skb)
{
...
#if defined(CONFIG_NF_NAT) || defined(CONFIG_NF_NAT_MODULE)
#if defined(CONFIG_NF_NAT_SINGLE_UDP_CT_FOR_LAN_WAN_TUPLE)
	struct nf_conn *sudp_rm_ct = NULL;
#endif
#if defined(CONFIG_NF_NAT_CONENAT_FAST_SEARCH)
	...
	if (sysctl_snatp2p_range_port_full_control && (ct->status & IPS_RANGE_PORT_FULL)) {
...
#if defined(CONFIG_NF_NAT_SINGLE_UDP_CT_FOR_LAN_WAN_TUPLE)
#if defined(CONFIG_NF_CONNTRACK_NAT_MANAGEMENT)
	if (sudp_rm_ct == drop_ct)
		drop_ct = NULL;
#endif
	if (sudp_rm_ct) {
		if (del_timer(&sudp_rm_ct->timeout))
			death_by_timeout((unsigned long)sudp_rm_ct);
		nf_ct_put(sudp_rm_ct);
	}
#endif
#endif
#if defined(CONFIG_NF_CONNTRACK_NAT_MANAGEMENT)
	if (drop_ct) {
		if (del_timer(&drop_ct->timeout))
			death_by_timeout((unsigned long)drop_ct);
		nf_ct_put(drop_ct);
	}
#endif
