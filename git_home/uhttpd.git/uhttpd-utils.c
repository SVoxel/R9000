 ...
 * example 1: dni_system(NULL, NULL, NULL, R_NORMAL, "/bin/ls", NULL);
 * example 2: dni_system("/tmp/result", NULL, NULL, R_NORMAL, "/bin/ls", "-l", NULL);
 */
#ifndef R_NORMAL
#define R_NORMAL 0
...
int dni_system(const char *output, const char *output2, unsigned char mode, const char *cmd, ...)
{
#define MAX_ARG 100
	int wait_val, pid;
...
int uh_cgi_auth_check(
	...
	if( (plen >= rlen) && !strncasecmp(pi->name, cl->server->conf->cgi_prefix, rlen) )
	{
		...
		if(pass != NULL)
		{
			...
			//system(command);
			dni_system("/tmp/hash_result",NULL,R_NORMAL,"/usr/sbin/hash-data","-e",pass,NULL);
