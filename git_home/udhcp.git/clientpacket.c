 ...
 * ---------------------------------------------------------
 * DHCPDECLINE packet format options:
 *
 * DHCP message type --- DHCPDECLINE
 * Requested IP address --- MUST
 * Server identifier --- MUST
 * IP address lease time --- MUST NOT
 * Vendor class identifier --- MUST NOT
 * Parameter request list --- MUST NOT
 * ......
 */
int send_decline(unsigned long xid, unsigned long requested, unsigned long svrid)
{
	struct dhcpMessage packet;
	...
	if (requested)
		add_simple_option(packet.options, DHCP_REQUESTED_IP, requested);
	if (svrid)
		add_simple_option(packet.options, DHCP_SERVER_ID, svrid);
