...
int ip_options_echo(struct ip_options *dopt, struct sk_buff *skb)
{
	...
	unsigned char *sptr, *dptr,*ddptr;
	int soffset, doffset;
	int	optlen,optlen1,type;
	...
	if (sopt->srr) {
		...
		__be32 faddr;
		/*type = start[0];
		if(type != IPOPT_LSRR || type != IPOPT_LSRR)
			return -EINVAL;*/
		...
		soffset -= 4;
		if(soffset > 0)
			optlen1 = dopt->optlen + (soffset /4) * 4 + 3; 
		else 
			optlen1 = dopt->optlen;
		if (optlen1 > 39)
			return -EINVAL;
