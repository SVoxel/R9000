...
static int show_dram_config(void)
{
	...
	for (i = size = 0; i < CONFIG_NR_DRAM_BANKS; i++) {
		...
		debug("Bank #%d: %08llx ", i, gd->bd->bi_dram[i].start);
#ifdef DEBUG
...
#  endif
	 ...
	 */
# if defined(CONFIG_ARM) && !defined(CONFIG_ARM64)
	gd->start_addr_sp -= CONFIG_STACKSIZE_ABORT;
	debug("Reserving %zu Bytes for abort stack at: %08lx\n",
		CONFIG_STACKSIZE_ABORT, gd->start_addr_sp);
