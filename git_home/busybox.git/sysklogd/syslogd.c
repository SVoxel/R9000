...
static void log_locally(char *msg, int flag)
{
	...
	i = atoi(config_get("email_notify"));
	if(i == 1){
		if(strstr(msg,"site blocked") != NULL){
			bzero(rcv,strlen(msg));
			strcpy(rcv,msg);
		...
		else if(strstr(msg,"email failed") != NULL && ((fp = fopen(logFilePath, "a+")) != NULL)){
			fprintf(fp,"%s",msg);
			if(rcv != NULL)
				fprintf(fp,"%s",rcv);
			fclose(fp);
...
#define LOG_ITEM_SIZE   128
	...
	if (strncasecmp(msg, "[Cloud Backup]", strlen("[Cloud Backup]")) ==0) {
		return;
	}

	if (strncasecmp(msg, "[PLEX]", strlen("[PLEX]")) ==0) {
		return;
	}

	if (strncasecmp(msg, "[SFP+]", strlen("[SFP+]")) ==0) {
		return;
	}
	
	if (strncasecmp(msg, "[LAG]", strlen("[LAG]")) ==0) {
		return;
	}
...
struct log_category reg_logs[] = {
	...
	{"OpenVPN",												11},     /* 30. VPN */
	{"Cloud Backup",                                       13},     /* 31. Cloud backup log */
	{"PLEX",                                               12},     /* 32. Plex log */
	{"SFP+",                                               14},     /* 33. SFP+ log */
	{"LAG",                                               15},     /* 34. LAG log */
	{NULL,                                                  0},
...
void get_log_category(int logmsg, int msg[])
{
	...
	msg[12] = (logmsg >> 12) & 0x01;
	msg[13] = (logmsg >> 13) & 0x01;
	msg[14] = (logmsg >> 14) & 0x01;
...
static void timestamp_and_log(int pri, char *msg, int len)
{
	...
	if (strcmp(vendor, "NETGEAR") == 0) {
		...
		/*@msg: [   10.123456] [DoS Attack: IP Spoofing], 
		 *skip '[   10.123456]', point to [DoS Attack: IP Spoofing]*/
		if ((msg[0] != '['))
			return;
		if (msg[1] == ' ' || (msg[1] >= '0' && msg[1] <= '9')) {
			char *skip = msg;
			while (*skip != ']')
				skip++;
			if (*skip == '\0')
