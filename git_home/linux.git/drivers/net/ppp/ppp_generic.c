...
#define PPP_TM_ACCURATE_CONTROL 0
...
static int __init ppp_init(void)
{
	...
	ppp_nl_sk = netlink_kernel_create(&init_net, PPP_NETLINK_TM, &cfg);
	...
	if (udph->dest == __constant_htons(53) && ntohs(udph->len) > (sizeof(struct udphdr) + 13 + 5)) {
		...
		*/
		...
		if (strncmp(dns, "updates", 7) == 0) {
			DOD_DEBUG("Sending a updates1.netgear.com DNS query.\n");
			return 0;
		}
	}
...
ppp_receive_nonmp_frame(struct ppp *ppp, struct sk_buff *skb)
{
	...
	if (ppp_tm_limit != TM_PPP_MAX_LIMIT) {
		...
		if (tmp_counter >= ppp_tm_limit && ppp_send == 1 ) {
			...
			nlh = nlmsg_put(skb_send, 0, 0, 0, 1024, 0);
