...
static int check_skip_len_old(nand_info_t *nand, loff_t offset, size_t length)
{
	size_t len_excl_bad = 0;
	int ret = 0;
...
int nand_write_skip_bad_old(nand_info_t *nand, loff_t offset, size_t *length,
			u_char *buffer, int flags)
{
	...
	need_skip = check_skip_len_old(nand, offset, *length);
	if (need_skip < 0) {
		printf("Attempt to write outside the flash area\n");
		*length = 0;
		return -EINVAL;
...
int nand_read_skip_bad_old(nand_info_t *nand, loff_t offset, size_t *length,
		       u_char *buffer)
{
	int rval;
	size_t left_to_read = *length;
