...
#define USB_SMB_NAME	"NETGEAR R9000"
...
#define USB_MAP_TABLE "/tmp/plexmediaserver/.usb_map_table"
...
static void load_share_info(FILE *fp, char *diskname)
{
	...
	*/
	...
	list_for_each(pos, &disk_lists) {
		...
		for(j=0; ;j++){
			...
			if (share_name == NULL ||folderName == NULL ||
					...
					continue;			
			/* Fix bug 66866, some no volume name partition will show no share in UI,just device_id is enough for determine the uniqueness of partition */
			if(strcmp(device_id,diskinfo->device_id) || strcmp(deviceName,diskinfo->vendor))
					continue;
			...
			if(config_match("factory_mode","1" )){
				...
				snprintf(cmd, sizeof(cmd), 
						"ls -l /sys/block/%s | grep usb | awk \'{print $11}\' | awk -F/ \'{print $9}\' | cut -d \"-\" -f 2", 
						disk);
				fp = popen(cmd, "r");
				if (!fp) {
					perror("popen");
	...
	if (no_shareinfo && diskname != NULL) {
		...
		char *serial_n, *part_id, disk_id[128], line[256], *sep;
		...
		list_for_each(pos, &disk_lists) {
			...
			for(i=0;i<100 ;i++){
				...
				//Fix bug TD710,for some multi-partition drive,sometimes GUI will default show not shared
				else 
				{
					sep = "*\n";
					strcpy(line,val);
					strtok(line, sep);/* share name */
					strtok(NULL, sep);	/* folder name */
					strtok(NULL, sep);	/* readaccess */
					strtok(NULL, sep);	/* writeaccess */
					strtok(NULL, sep);	/* volume name */
					strtok(NULL, sep);	/* device name */
					serial_n = strtok(NULL, sep);	/* serial number */
					part_id = strtok(NULL, sep);	/* partition id */
					snprintf(disk_id, sizeof(disk_id), "%s*%s", serial_n, part_id);
					if(strcmp(disk_id, diskinfo->device_id) == 0)
					{
			...
			config_set(name,value);
		#endif 
			snprintf(folder_path, sizeof(folder_path), "/mnt/%s/", diskinfo->mount_name);
