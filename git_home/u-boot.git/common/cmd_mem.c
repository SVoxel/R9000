...
static int do_mem_pattern_set(
		cmd_tbl_t *cmdtp, int flag, int argc, char * const argv[])
{
	ulong addr;
	ulong count;
	unsigned long long write_val[2]; /* even, odd */
	void *buf;
	ulong bytes;
	int size = sizeof(unsigned long long) / sizeof(uchar); /* 8 */
	int i;
	...
	/* Get the value to write [even, odd]. */
	write_val[0] = simple_strtoull(argv[3], NULL, 16);
	write_val[1] = simple_strtoull(argv[4], NULL, 16);

	bytes = size * count;
	buf = map_sysmem(addr, bytes);
	for (i = 0; i < count; ++i) {
		*((unsigned long long *)buf) =
				(unsigned long long)write_val[i%2];
		buf += size;
	}
	unmap_sysmem(buf);
	return 0;
...
U_BOOT_CMD(
    ...
    defined(CONFIG_HW29765294P0P128P512P8X8P4X4)
U_BOOT_CMD(
	mem_pattern_set, 5, 0, do_mem_pattern_set,
	"set values alternately according to the even and odd values",
	"<start address> <(64)count> <even 64 bit value> <odd 64 bit value>"
);
#endif
