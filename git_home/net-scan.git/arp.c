		...
		for (pprev = &arp_tbl[i], u = *pprev; u; ) {
			...
			if (sb_fp){
				...
				if(sb_dlp){
					...
					inet_ntoa(u->ip),  mac, sb_dlp->type == TYPE_SOAP_OLD ? type : sb_dlp->type, netgear_priority,
					...
					fprintf(sb_fp, "%s %s %d %s %.2f %.2f %s @#$&*!\n",
					inet_ntoa(u->ip), mac, type, netgear_priority,
					u->state == SB_INFO_STATE_2? u->down : 0.0,
					u->state == SB_INFO_STATE_2? u->up : 0.0,
					u->host[0] == '\0' ? "&lt;unknown&gt;" : u->host);
				}
