					 ...
					 unsigned int speed)
{
	...
	if (speed == I2C_MAX_SPEED)
		...
		printf("Invalid speed - supported speeds (%u, %u, %u)\n",
			I2C_STANDARD_SPEED, I2C_FAST_SPEED, I2C_MAX_SPEED);
		return 1;
	}
#endif
...
#if defined(CONFIG_HW29765235P0P512P1024P4X4P4X4) || \
    defined(CONFIG_HW29765294P0P128P512P8X8P4X4)
		int status = readl(&i2c_base->ic_status);
		/* For new read xfer: issue read command */
		if ((tx_bytes_cnt > 0) && (status & IC_STATUS_TFNF) && !(status & IC_STATUS_RFF)) {
