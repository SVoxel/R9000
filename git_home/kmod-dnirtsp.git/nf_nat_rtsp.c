                 ...
                 struct sk_buff* skb, uint protoff, uint tranoff, uint tranlen)
{
	...
	char        buffer[sizeof("destination=nnn.nnn.nnn.nnn;")];
	unsigned    buflen = 0;
    u_int32_t   newip;
    ...
    case pb_range:
		...
		{
			exp->tuple.dst.u.udp.port = htons(loport);
			exp2->tuple.dst.u.udp.port = htons(loport + 1);
			if (nf_ct_expect_related(exp) == 0) {
            ...
            if (dstact != DSTACT_NONE && strncmp(ptran+off, "destination=", 12) == 0)
            {
                ...
                if (dstact == DSTACT_STRIP || (dstact == DSTACT_AUTO && !is_stun))
                {
					if (dstact == DSTACT_AUTO)
						buflen = sprintf(buffer, "destination=%u.%u.%u.%u;",
								((unsigned char *)&newip)[0],
								((unsigned char *)&newip)[1],
								((unsigned char *)&newip)[2],
								((unsigned char *)&newip)[3]);

					diff = nextfieldoff-off;
					if (!nf_nat_mangle_tcp_packet(skb, ct, ctinfo, protoff, 
                                            tranoff+off, diff, buffer, buflen))
                    {
	 ...
	 struct nf_conntrack_expect* exp, struct nf_conntrack_expect* exp2)
{
    ...
    while (nf_mime_nextline(ptcp, hdrsoff+hdrslen, &off, &lineoff, &linelen))
    {
	    ...
	    pr_debug("hdr: Transport\n");
            if (!rtsp_mangle_tran(ctinfo, exp, exp2, prtspexp, skb, protoff, lineoff, linelen))
            {
     ...
     struct nf_conntrack_expect* exp, struct nf_conntrack_expect* exp2)
{
    ...
    case IP_CT_DIR_ORIGINAL:
	    rc = help_out(skb, ctinfo, protoff, matchoff, matchlen, prtspexp, exp, exp2);
        break;
