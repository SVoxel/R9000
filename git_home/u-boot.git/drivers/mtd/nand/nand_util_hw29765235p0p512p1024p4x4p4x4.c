...
extern loff_t part_base_addr;
/**
 * change offset according to bad blocks from the beginning of the current partition.
 *
 * @param  nand   NAND device
 * @param  offset offset from start of the NAND device (input and output)
 * @return 0 in case of success
 */
static int get_bad_block_off_partition_base(nand_info_t *nand, loff_t *offset)
{
	loff_t new_offset = *offset;
	loff_t part = part_base_addr;

	while (part < new_offset) {
		...
		block_start = part & ~(loff_t)(nand->erasesize - 1);
		block_off = part & (nand->erasesize - 1);
	...
	if (new_offset != *offset) {
		debug("%s: change offset from %llx to %llx\n",
...
int nand_erase_opts(nand_info_t *meminfo, const nand_erase_options_t *opts)
{
	...
	if (opts->scrub) {
		...
		get_bad_block_off_partition_base(meminfo, (loff_t *)&erase.addr);
	}
		...
		size_t *used)
{
	...
	get_bad_block_off_partition_base(nand, offset);
	...
	while (len_excl_bad < length) {
		...
		if (temp_off >= nand->size)
			return -1;

		block_start = temp_off & ~(loff_t)(nand->erasesize - 1);
		block_off = temp_off & (nand->erasesize - 1);
		block_len = nand->erasesize - block_off;
 ...
 */
size_t get_len_incl_bad (nand_info_t *nand, loff_t offset,
				const size_t length)
{
