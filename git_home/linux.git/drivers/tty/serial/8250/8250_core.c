...
#define MAX_BUFF_COUNT 4096
char console_buf[4096];
unsigned int console_index = 0;
...
static ssize_t show_console(struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	if(console_index > 0 && console_index < MAX_BUFF_COUNT -2) {
	//strcpy(buf, console_buf);
	memcpy(buf, console_buf, console_index+1);
	len = console_index;
	console_index = 0;
	}
...
#ifdef CONFIG_DNI_CONSOLE_REDIRECT_DEBUG
	if(console_enable == 1){
		if (console_index == 0 || (console_index >= (MAX_BUFF_COUNT - 3))) {
		...
		if(!(value == 0x5 || value == 0x7 || value == 0x1B)) {
			console_buf[console_index] = (char)value;
			console_index++;
		}
	}
#endif
