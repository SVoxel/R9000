...
#define OPTS_WRITE	(1 << 6)
#define OPTS_BIST	(1 << 9)
...
int __weak ahci_link_up(struct ahci_probe_ent *probe_ent, u8 port)
{
	u32 tmp;
	int j = 0;
	u8 *port_mmio = (u8 *)(uintptr_t)probe_ent->port[port].port_mmio;
...
{
	int i = 1000;
	ulong host_ctl_reg = base + HOST_CTL;
...
static int ahci_host_init(struct ahci_probe_ent *probe_ent)
{
...
#endif
	volatile u8 *mmio = (volatile u8 *)(uintptr_t)probe_ent->mmio_base;
	...
	if (vendor == PCI_VENDOR_ID_ANNAPURNALABS) {
		pci_read_config_word(pdev, PCI_COMMAND, &tmp16);
		tmp16 |= PCI_COMMAND_MEMORY;
		pci_write_config_word(pdev, PCI_COMMAND, tmp16);
	}
#endif
	...
	for (i = 0; i < probe_ent->n_ports; i++) {
		if (!(port_map & (1 << i)))
			continue;
		probe_ent->port[i].port_mmio = ahci_port_base((u32)(uintptr_t)mmio, i);
		port_mmio = (u8 *)(uintptr_t)probe_ent->port[i].port_mmio;
...
static int ahci_fill_sg(u8 port, unsigned char *buf, int buf_len)
{
	...
	for (i = 0; i < sg_count; i++) {
		ahci_sg->addr =
		    cpu_to_le32((u32)(uintptr_t)buf + i * MAX_DATA_BYTE_COUNT);
		ahci_sg->addr_hi = 0;
...
{
	struct ahci_ioports *pp = &(probe_ent->port[port]);
	volatile u8 *port_mmio = (volatile u8 *)(uintptr_t)pp->port_mmio;
...
static int ahci_port_start(u8 port)
{
	...
	mem = (ulong) malloc(AHCI_PORT_PRIV_DMA_SZ + 2048);
	if (!mem) {
		free(pp);
	...
	debug("cmd_slot = 0x%x\n", (unsigned)(uintptr_t)pp->cmd_slot);
	...
	writel_with_flush((u32)(uintptr_t)pp->cmd_slot, port_mmio + PORT_LST_ADDR);
				...
				int buf_len, u16 opts_flags)
{
	...
	memcpy((unsigned char *)(uintptr_t)pp->cmd_tbl, fis, fis_len);

	sg_count = ahci_fill_sg(port, buf, buf_len);
	opts = (sg_count << 16) | opts_flags | (fis_len >> 2);
	ahci_fill_cmd_slot(pp, opts);

	ahci_dcache_flush_sata_cmd(pp);
	ahci_dcache_flush_range((unsigned)(uintptr_t)buf, (unsigned)(uintptr_t)buf_len);

	writel_with_flush(1, port_mmio + PORT_CMD_ISSUE);

	if ((opts_flags & OPTS_BIST) == 0) {
		if (waiting_for_cmd_completed(port_mmio + PORT_CMD_ISSUE,
					WAIT_MS_DATAIO, 0x1)) {
			printf("timeout exit!\n");
			return -1;
	...
	ahci_dcache_invalidate_range((unsigned)(uintptr_t)buf, (unsigned)(uintptr_t)buf_len);
...
int scsi_ata_bist(ccb *pccb)
{
	...
	u8 port = pccb->target;
	u8 mode = pccb->cmd[1];
	u8 pattern = pccb->cmd[2];
	u32 testr, bistcr;
	volatile u8 *mmio = (volatile u8 *)(uintptr_t)probe_ent->mmio_base;

	debug("%s: activating Built-In Self Test\n", __func__);

	/* Setting BIST device, according to current device */
	debug("%s: setting BIST device to 0x%x\n", __func__, port);
	testr = readl(mmio + HOST_TESTR);
	testr = (testr & 0xFFF8FFFF) | HOST_TESTR_PSEL(port);
	writel(testr, mmio + HOST_TESTR);

	/* Setting BIST pattern */
	debug("%s: setting BIST pattern to 0x%x\n", __func__, pattern);
	bistcr = readl(mmio + HOST_BISTCR);
	writel((bistcr & 0xFFFFFFF0) | pattern, mmio + HOST_BISTCR);

	/* Preset the FIS */
	memset(fis, 0, sizeof(fis));
	fis[0] = 0x58;	/* BIST FIS */
	fis[2] = mode;	/* BIST Mode */
	debug("%s: setting BIST mode to 0x%x\n", __func__, mode);

	/* Read/Write from ahci */
	ahci_device_data_io(pccb->target, (u8 *) &fis, sizeof(fis),
				user_buffer, user_buffer_size,
				OPTS_BIST);
	return 0;
}
...
{
	volatile u8 *mmio = (volatile u8 *)(uintptr_t)probe_ent->mmio_base;
	u32 *data = (u32 *)pccb->pdata;

	data[0] = readl(mmio + HOST_BISTFCTR);
	data[1] = readl(mmio + HOST_BISTSR);
	data[2] = readl(mmio + HOST_BISTDECR);
...
static int ata_io_flush(u8 port)
{
	...
	memcpy((unsigned char *)(uintptr_t)pp->cmd_tbl, fis, 20);
	ahci_fill_cmd_slot(pp, cmd_fis_len);
	writel_with_flush(1, port_mmio + PORT_CMD_ISSUE);
