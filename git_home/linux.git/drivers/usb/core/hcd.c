...
/*-------------------------------------------------------------------------*/
extern void detect_usb1_data(void);
extern void detect_usb3_data(void);
...
int usb_hcd_submit_urb (struct urb *urb, gfp_t mem_flags)
{
	...
	if (is_root_hub(urb->dev)) {
		...
		if (likely(status == 0)) {
			if ((strcmp(dev_name(&urb->dev->dev), "1-1") == 0) || (strcmp(dev_name(&urb->dev->dev), "2-1") == 0))
				detect_usb1_data();
			else if ((strcmp(dev_name(&urb->dev->dev), "1-2") == 0) || (strcmp(dev_name(&urb->dev->dev), "2-2") == 0))
				detect_usb3_data();
