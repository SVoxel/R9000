...
static int l2tp_recv_core(struct l2tp_tunnel *tunnel, struct sk_buff *skb)
{
	...
	if (!pskb_may_pull(skb, ptr - skb->data))
		goto discard;
...
static int l2tp_xmit(struct ppp_channel *chan, struct sk_buff *skb)
{
	...
	rt = ip_route_output_key(sock_net(sk), &po->fl);
	if (IS_ERR(rt)) {
		rt = NULL;
		goto done;
	}
	...
	u32_set(&iph->saddr, po->fl.saddr);
	u32_set(&iph->daddr, po->fl.daddr);
#else
	iph->saddr = po->fl.saddr;
	iph->daddr = po->fl.daddr;
#endif
#else
#ifdef CONFIG_IP_NF_IP_ALIGNMENT
