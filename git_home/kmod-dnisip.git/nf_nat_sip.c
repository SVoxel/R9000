...
next:
	...
	if (!map_sip_addr(skb, protoff, dataoff, dptr, datalen, SIP_HDR_FROM) ||
		...
		!map_sip_addr(skb, protoff, dataoff, dptr, datalen, SIP_HDR_RECORD_ROUTE) ||
	    !map_sip_addr(skb, protoff, dataoff, dptr, datalen, SIP_HDR_ROUTE)) {
...
static int expect_related_rtp_rtcp(u_int16_t port, struct nf_conntrack_expect *rtp_exp, 
...
static int try_expect_related_rtp_rtcp(u_int16_t *port,
		struct nf_conntrack_expect *rtp_exp, 
		struct nf_conntrack_expect *rtcp_exp)
{
	u_int16_t base_port = *port;

	for (*port = base_port; *port != 0; *port += 2)
		if (expect_related_rtp_rtcp(*port, rtp_exp, rtcp_exp) == 0 )
			return 0;
	for (*port = base_port - 2; *port != 1024; port -= 2)
		if (expect_related_rtp_rtcp(*port, rtp_exp, rtcp_exp) == 0)
			return 0;
	return -1;
}
				     ...
				     union nf_inet_addr *rtp_addr)
{
	...
	if (try_expect_related_rtp_rtcp(&port, rtp_exp, rtcp_exp) < 0 ) {
...
static int __init nf_nat_sip_init(void)
{
	...
	rcu_assign_pointer(nf_nat_sip_seq_adjust_hook, nf_nat_sip_seq_adjust);
	rcu_assign_pointer(nf_nat_sip_expect_hook, nf_nat_sip_expect);
	rcu_assign_pointer(nf_nat_sdp_addr_hook, nf_nat_sdp_addr);
	rcu_assign_pointer(nf_nat_sdp_port_hook, nf_nat_sdp_port);
	rcu_assign_pointer(nf_nat_sdp_session_hook, nf_nat_sdp_session);
	rcu_assign_pointer(nf_nat_sdp_media_hook, nf_nat_sdp_media);
	return 0;
}
