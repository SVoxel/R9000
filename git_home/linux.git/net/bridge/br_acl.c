...
void __init br_acl_init(void)
{
	...
	dnsnl = netlink_kernel_create(&init_net, NETLINK_USERSOCK,&cfg);
}
...
void br_acl_cleanup(struct net_bridge *br)
{
	...
	for (i = 0; i < BR_ACL_HASH_SIZE; i++) {
		...
		hlist_for_each_entry_safe(acl, h, &br->acl_hash[i], hlist)
			acl_delete(acl);
	}
...
int wigig_acl_should_pass(struct net_device *dev, struct sk_buff *skb)
{
...
EXPORT_SYMBOL(wigig_acl_should_pass);
