...
function check_pppoe(form,check)
{
	...
	if(parent.enable_vlan_pppoe_support == 0 && vlan_iptv_enable == "1"){
		if(confirm("$vlan_pppoe_support_query") == false)
			return false;
	}
	if( form.pppoe_dod.value == "1" || form.pppoe_dod.value == "2" )
	{
...
function check_orange_pppoe(form,check)
{
	...
	if(check_wizard_pppoe(check,"","bas")==false)
		return false;
	form.hidden_pppoe_idle_time.value = form.pppoe_idletime.value;
	form.hid_pppoe_dod.value = form.pppoe_dod.value;//bug 22322:when connection mode gray out,need a hidden-value to transfer
	if(form.WANAssign[1].checked == true)
	...
	if(form.orange_enable.checked == true)
...
function check_orange_dhcp(form,check)
{
	...
	{
		if(isValidChar(form.orange_username.value.charCodeAt(i))==false)
		{
			alert("$loginname_not_allowed");
			return false;
		}
