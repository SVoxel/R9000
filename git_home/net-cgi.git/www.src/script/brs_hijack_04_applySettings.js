function initPage()
{
	...
	else
	{
		if(top.dsl_enable_flag == "1")
		{
			...
			if(top.location.href.indexOf("BRS_index.htm") > -1 && top.orange_apply_flag != "1"){
				if(top.location.href.indexOf("routerlogin.net") > -1){
					setTimeout("top.location.href='http://routerlogin.com/BRS_index.htm';", 75000);
				}else{
					setTimeout("top.location.href='http://routerlogin.net/BRS_index.htm';", 75000);
				}
			}else
				setTimeout("this.location.href='BRS_04_applySettings_ping.html';", 75000);
				top.orange_apply_flag = "0";
		}
	}
}
