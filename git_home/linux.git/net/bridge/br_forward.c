											...
											unsigned long len, void *data)
{
	...
	//printk("mcast_set_write: input buf is %s, len: %u\n", line, len);
...
{
	struct ethhdr *ehdr;
	struct iphdr *igmp_iph, _igmp_iph;
	unsigned char *igmp_type;
	igmpr_t *igmpv12_report;
	igmp_report_t *igmpv3_report, _igmpv3_report;
	...
	igmp_iph = skb_header_pointer(skb, 0, sizeof(_igmp_iph), &_igmp_iph);
	if (igmp_iph == NULL)
		return;

	if (MULTICAST_MAC(ehdr->h_dest) && igmp_iph->protocol == IPPROTO_IGMP && igmp_iph->daddr != SSDP)
	{
		iphlen = igmp_iph->ihl * 4;
		igmp_type = (char *)skb_header_pointer(skb, iphlen, sizeof(igmp_report_t), &_igmpv3_report);
		if(igmp_type == NULL)
			return;
		sip = igmp_iph->saddr;
...
static void custom_bootp_lease_time(struct sk_buff *skb)
{
 ...
 // udph = (struct udphdr *)(((unsigned char *)iph) + (iph->ihl<<2));
	udphlen = iph->ihl * 4;
	udph = (char *)skb_header_pointer(skb, udphlen, sizeof(_udph), &_udph);
	if(udph == NULL)
		return;
