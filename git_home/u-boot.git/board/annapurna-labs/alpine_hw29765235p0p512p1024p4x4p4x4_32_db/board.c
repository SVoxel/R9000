...
int checkboard(void)
{
	printf("U-boot dni1 V1.9 for DNI HW ID: 29765235 NOR flash 0MB; NAND flash 512MB; RAM 1024MB; 1st Radio 4x4; 2nd Radio 4x4\n");
	return 0;
}
#endif
...
int misc_init_r(void)
{
	...
	{
		char *sfp_command = "serdes tx_params_set 3 3 1 21 4 0 0";

		printf("runcommand %s..." , sfp_command);
		if (run_command(sfp_command, 0) != CMD_RET_SUCCESS) {
			printf ("fails\n");
		} else {
			printf ("done\n");
		}
...
int do_ledtoggle (cmd_tbl_t *cmdtp, int flag, int argc, char *argv[])
{
    int val;
    ...
    if( gpio_get_value(POWER_LED) ) {
        gpio_set_value(POWER_LED,0);
        printf("POWER_LED on --> off \n");
    } else {
        printf("POWER_LED off --> on\n");
        gpio_set_value(POWER_LED,1);
    }
...
U_BOOT_CMD(
        ledtoggle,2, 0, do_ledtoggle,
        "ledtoggle - Test LEDs\n",
        "\nledtoggle: LED toggle between on/off"
);
