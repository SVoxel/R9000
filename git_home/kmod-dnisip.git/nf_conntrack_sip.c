...
static const struct sip_header ct_sip_hdrs[] = {
	...
	[SIP_HDR_RECORD_ROUTE]      = SIP_HDR("Record-Route", NULL, "sip:", skp_epaddr_len),
	[SIP_HDR_ROUTE]         = SIP_HDR("Route", NULL, "sip:", skp_epaddr_len),
};
...
static void flush_expectations(struct nf_conn *ct, bool media)
{
	...
	hlist_for_each_entry_safe(exp, next, &help->expectations, lnode) {
		if (((exp->class != SIP_EXPECT_SIGNALLING) &&
			(exp->class != SIP_EXPECT_VIA)) ^ media)
			continue;
		if (!del_timer(&exp->timeout))
			continue;
				 ...
				 unsigned int mediaoff, unsigned int medialen)
{
	...
	union nf_inet_addr *saddr, nulladdr;
	...
	do {
		...
		if (!exp || exp->master == ct ||
			nf_inet_addr_cmp(&exp->saved_addr, &nulladdr) ||
	...
	rtp_exp->saved_addr = nulladdr;
	rtp_exp->saved_proto.udp.port = 0;
...
static int process_via(struct sk_buff *skb, unsigned int protoff, unsigned int dataoff,
		       const char **dptr, unsigned int *datalen)
{
	enum ip_conntrack_info ctinfo;
	struct nf_conn *ct = nf_ct_get(skb, &ctinfo);
	...
	if ((ct_sip_parse_header_uri(ct, *dptr, NULL, *datalen,
	     hdr, NULL, &matchoff, &matchlen, &daddr, &port) > 0) &&
	    (nf_inet_addr_cmp(&ct->tuplehash[dir].tuple.src.u3, &daddr)) && 
		(ct->tuplehash[dir].tuple.src.u.udp.port != port)) {
		if (!(exp = nf_ct_expect_alloc(ct)))
			return NF_DROP;

		nf_ct_expect_init(exp, SIP_EXPECT_VIA, nf_ct_l3num(ct),
			  saddr, &daddr, proto, NULL, &port);
		exp->timeout.expires = 60 * HZ;
		exp->helper = nfct_help(ct)->helper;

		nf_nat_sip_expect = rcu_dereference(nf_nat_sip_expect_hook);
		if (nf_nat_sip_expect && ct->status & IPS_NAT_MASK)
			ret = nf_nat_sip_expect(skb, protoff, dataoff, dptr, datalen, exp,
					matchoff, matchlen);
		else {
...
static int process_via_sdp(struct sk_buff *skb, unsigned int protoff, unsigned int dataoff, 
		       const char **dptr, unsigned int *datalen,
		       unsigned int cseq)
{
	if (process_via(skb, protoff, dataoff, dptr, datalen) == NF_DROP)
		return NF_DROP;
				  ...
				  unsigned int cseq)
{
	...
	flush_expectations(ct, true);
	ret = process_via_sdp(skb, protoff, dataoff, dptr, datalen, cseq);
	if (ret == NF_ACCEPT)
...
static const struct sip_handler sip_handlers[] = {
	...
	SIP_HANDLER("UPDATE", process_via_sdp, process_update_response),
	SIP_HANDLER("ACK", process_via_sdp, NULL),
	SIP_HANDLER("PRACK", process_via_sdp, process_prack_response),
