...
function sAlert(str, callback_ok, callback_cancel, dwidth, anc, button_lang){
	...
	else if(button_lang == "cancel/apply" || button_lang == "cancel/ok")
	...
	else if(button_lang != "")
	{
		if(pattion == -1)
			...
			button.setAttribute("value", button_lang.substring(0, pattion-1));
	}
	else
	...
	if(this.location.href.indexOf("plex_") != -1 || this.location.href.indexOf('Netdisk_') != -1)
		var oldSty = 1;
	if(button_lang == "cancel/apply" || button_lang == "cancel/ok")
	{
		if(oldSty == 1) {
	...
	else if(button_lang == "cancel/ok")
		button1.setAttribute("value","$ok_mark");
	else if(button_lang != "")
	{
		if(pattion == -1)
			button.setAttribute("value", button_lang);
		else
			button.setAttribute("value", button_lang.substring(pattion+1));
	}
	else
	...
	function click_cancel(){// for cancel button
		if(typeof(callback_cancel) == "function")
			callback_cancel();
		if(button_lang != "yes/no" && button_lang != "cancel/apply" && button_lang != "cancel/ok")
			removeObj();
	}
...
{
	please_wait((flag === undefined? true: flag));
	...
	else
	{
		...
		for(i=0;i<passphrase.value.length;i++)
		{
			if(arguments[2] == "ad")
			{
				...
				{
					alert("60G wireless passphrase doesn't support these characters \"&'()*;<>");
					return false;
				}
			}
...
*/
	if( cf.login_type.value == "PPTP" )
		goto_formframe("BAS_pptp.htm", false);
	else if( cf.login_type.value == "L2TP" )
		goto_formframe("BAS_l2tp.htm", false);
	else if( cf.login_type.value == "Telstra Bigpond" )
		goto_formframe("BAS_bpa.htm", false);
	else if( cf.login_type.value == "PPPoE" )
	{
	   if(top.dsl_enable_flag == 0)
		goto_formframe("BAS_pppoe.htm", false);
	   else
		goto_formframe("BAS_pppoe_dsl.htm", false);
	}
	else if(cf.login_type.value == "multiPPPoE" )
	{
		if(top.netgear_region != "JP")
			goto_formframe("BAS_mulpppoe.htm", false);
		else
			goto_formframe("BAS_mulpppoe_ww.htm", false);
	}
	else if( cf.login_type.value == "Orange PPPoE" )
		goto_formframe("BAS_orange_pppoe.htm");
	else if( cf.login_type.value == "Orange DHCP" )
	...
	switch(type)
	{
		...
		case "6rd":
			top.formframe.location.href = "IPv6_6rd.htm";
                        break;
		case "orange":
			top.formframe.location.href = "IPv6_orange.htm";
			break;
		default:
			top.formframe.location.href = "IPv6_disabled.htm";
			break;
