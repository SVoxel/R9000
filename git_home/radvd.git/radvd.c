...
char usage_str[] =
	...
	"\t[-f facility] [-p pid_file] [-u username] [-t chrootdir] [-x max_init_router_advertisements]";
...
struct option prog_opt[] = {
	...
	{"help", 0, 0, 'h'},
	{"singleprocess", 0, 0, 's'},
	{"max-init-router-advertisements", required_argument, 0, 'x'},
	{NULL, 0, 0, 0}
};
#endif
...
int max_init_router_advertisements = MAX_INITIAL_RTR_ADVERTISEMENTS - 1;
...
main(int argc, char *argv[])
{
	...
	/* parse args */
#ifdef HAVE_GETOPT_LONG
	while ((c = getopt_long(argc, argv, "d:C:l:m:p:t:u:vhsx:", prog_opt, &opt_idx)) > 0)
#else
	while ((c = getopt(argc, argv, "d:C:l:m:p:t:u:vhsx:")) > 0)
#endif
	{
		switch (c) {
		case 'C':
		...
		case 's':
			...
			break;
		case 'x':
			max_init_router_advertisements = atoi(optarg) - 1;
			break;
		case 'h':
			usage();
#ifdef HAVE_GETOPT_LONG
	...
	/* get system time */
	start_time = uptime();
	prefix_consume = should_decrease();
...
timer_handler(void *data)
{
	...
	if (iface->init_racount < max_init_router_advertisements)
	{
		iface->init_racount++;
...
sigusr1_handler(int sig)
{
	...
	dlog(LOG_DEBUG, 4, "sigusr1_handler called, should reset uptime");
