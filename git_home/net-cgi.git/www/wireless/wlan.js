...
function setSecurity_ad(num)
{
	...
	form.wig_wpas_press_flag.value=0;
	...
	if(num==1)
		...
		getObj("view_ad").innerHTML = str_wpas_ad;
	else if(num==6)
	{
		getObj("view_ad").innerHTML = str_wpae_ad;
		form.wpae_mode_ad.value = get_wpae_mode_d;
		if( get_radiusSerIp_d != "" && get_radiusSerIp_d != "0.0.0.0" )
		{
			radiusIPArray = get_radiusSerIp_d.split(".");
			form.radiusIPAddr1_ad.value = radiusIPArray[0];
			form.radiusIPAddr2_ad.value = radiusIPArray[1];
			form.radiusIPAddr3_ad.value = radiusIPArray[2];
			form.radiusIPAddr4_ad.value = radiusIPArray[3];
		}
		form.textWpaeRadiusPort_ad.value = get_radiusPort_d;
	}
}
...
*/
	...
	if(ad_router_flag == 1)
	{
		...
		if(cf.security_type_ad[2].checked == true)
		{
			if(checkpsk(cf.passphrase_ad, cf.wig_sec_wpaphrase_len, "ad") == false)
				return false;
			cf.wig_hidden_sec_type.value = 5;
			cf.wig_hidden_wpa_psk.value = cf.passphrase_ad.value;
		}
		if(cf.security_type_ad[3].checked == true)
		{
			cf.wlg_hidden_wlan_mode.value = cf.wpae_mode_ad.value;

			radiusServerIP = cf.radiusIPAddr1_ad.value+'.'+ cf.radiusIPAddr2_ad.value + '.' + cf.radiusIPAddr3_ad.value + '.' + cf.radiusIPAddr4_ad.value;
			if( radiusServerIP == "" || checkipaddr(radiusServerIP) == false )
			{
				 alert("$invalid_ip");
				  return false;
			...
			cf.radiusServerIP_d.value = radiusServerIP;

			radiusPort=parseInt(cf.textWpaeRadiusPort_ad.value,10);
			if( isNaN(radiusPort) || radiusPort < 1 || radiusPort > 65535 )
			{
				alert("$radiusPort65535");
				return false;
			}
			cf.textWpaeRadiusPort_ad.value=radiusPort;
			cf.textWpaeRadiusPort_ad.value=port_range_interception(cf.textWpaeRadiusPort_ad.value);

			if( cf.textWpaeRadiusSecret_ad.value == "" )
			{
				alert("$radiusSecret128");
				return false;
			}
			if( cf.textWpaeRadiusSecret_ad.length > 128 )
			{
				alert("$radiusSecret128");
				return false;
			}
			for(i=0;i<cf.textWpaeRadiusSecret_ad.value.length;i++)
			{
				if(isValidChar(cf.textWpaeRadiusSecret_ad.value.charCodeAt(i))==false)
				{
					alert("$radiusSecret128");
					cf.textWpaeRadiusSecret_ad.focus();
					return false;
				}
			}
			cf.hidden_WpaeRadiusSecret_d.value = cf.textWpaeRadiusSecret_ad.value;
			cf.wig_hidden_sec_type.value=6;
		}
	}
...
var ht20_array = new Array(
	...
	new Array ( "36", "40", "44", "48", "149", "153", "157", "161", "165" ), //11
...
var ht40_array = new Array(
	...
	new Array ( "36", "40", "44", "48", "149", "153", "157", "161" ), //11
...
var ht80_array = new Array(
	...
	new Array ( "36", "40", "44", "48" ), //19
	new Array ( "36", "40", "44", "48", "52(DFS)", "56(DFS)", "60(DFS)", "64(DFS)", "149", "153", "157", "161" ), //20
	new Array ( "100(DFS)", "104(DFS)", "108(DFS)", "112(DFS)", "116(DFS)", "120(DFS)", "124(DFS)", "128(DFS)", "149", "153", "157", "161" ) //21
);
