...
lookup_phy() {
	...
	[ -n "$devpath" -a -d "/sys/devices/$devpath/ieee80211" ] && {
		phy="$(ls /sys/devices/$devpath/ieee80211 | grep phy | head -n 1)"
		[ -n "$phy" ] && return
	}
...
disable_mac80211() (
	...
	for wdev in $(list_phy_interfaces "$phy"); do
		...
		if eval "type unbridge" 2>/dev/null >/dev/null; then
			unbridge "$dev"
		fi
		iw dev "$wdev" del
	done
	...
	config_get hwmode "$device" hwmode
	if [ "$hwmode" = "11ad" ]; then
	    rm /tmp/WLAN_uptime_60G
	    echo "OFF" > /tmp/WLAN_60G_status
	fi
	return 0
)
...
get_freq() {
	local phy="$1"
	local chan="$2"
	iw "$phy" info | grep -E "(\* ${chan:-....} MHz${chan:+|\\[$chan\\]})" | head -n 1 | grep MHz | awk '{print $2}'
}
...
statistic_mac80211()
{
    ...
    local tx_bytes=0
    local rx_bytes=0
    local tx_packets_tmp=0
    local rx_packets_tmp=0
    local collisions_tmp=0
    local tx_bytes_tmp=0
    local rx_bytes_tmp=0
	...
	[ -n "$ifname" ] || {
        ...
        tx_packets=$(( $tx_packets + $tx_packets_tmp ))
        rx_packets=$(( $rx_packets + $rx_packets_tmp ))
        tx_bytes=$(( $tx_bytes + $tx_bytes_tmp ))
        rx_bytes=$(( $rx_bytes + $rx_bytes_tmp ))
	i=$(($i + 1))
    done
    echo "###60G###"
...
enable_mac80211() {
	...
	if [ "$hwmode" = "ad" ]; then
		qca_11ad_irq=`cat /proc/interrupts | grep wil6210 | awk -F: '{print $1}'`
		no_of_cpu=`grep processor /proc/cpuinfo | wc -l`
		if [ "$no_of_cpu" -eq 4 -a -n "$qca_11ad_irq" ]; then
			cur_smp_affinity=`cat /proc/irq/$qca_11ad_irq/smp_affinity`
			if [ "$cur_smp_affinity" = "f" ]; then
				echo 6 > /proc/irq/$qca_11ad_irq/smp_affinity
			fi
		fi
		cat /proc/uptime | sed 's/ .*//' > /tmp/WLAN_uptime_60G
		echo "ON" > /tmp/WLAN_60G_status
	fi
}
...
detect_mac80211() {
	...
	for dev in $(ls /sys/class/ieee80211); do
		...
		cat <<EOF
config wigig-device  radio$devidx
...
wigigtoggle_mac80211()
{
	local device="$1"

	# Wi-Fi button state BEFORE Wi-Fi button is pressed
	local old_hw_btn_state="$2"
...
wigigschedule_mac80211()
{
	...
	find_mac80211_phy "$device" || return 1

	if [ "$newstate" = "on" -a "$hw_btn_state" = "on" ]; then
		enable_mac80211 "$device"
		logger "[Wireless signal schedule] The wireless 60GHz signal is ON,"
	else
		disable_mac80211 "$device"
		logger "[Wireless signal schedule] The wireless 60GHz signal is OFF,"
	fi
}

wigigstainfo_mac80211() {
	local device="$1"
	...
	for vif in $vifs; do
		...
		# Insert empty lines between 11ad attached device records
		...
		[ -f /usr/lib/stainfo_11ad.awk ] && \
				awk -f /usr/lib/stainfo_11ad.awk $tmpfile
		rm $tmpfile
		echo ""
	done
}
