...
static inline int eth_setenv_enetaddr_by_index(const char *base_name, int index, uchar *enetaddr)
#endif
{
	char enetvar[32];
	sprintf(enetvar, index ? "%s%daddr" : "%saddr", base_name, index);
...
#if defined(CONFIG_HW29765235P0P512P1024P4X4P4X4) || \
    defined(CONFIG_HW29765294P0P128P512P8X8P4X4)
	/* NULL dev is used for skipping a device and not losing it's index */
	if (!dev) {
		index++;
		return 0;
	}
...
#ifndef CONFIG_NET_DO_NOT_TRY_ANOTHER
