...
do_stop() {
	...
	sync
	sync
	#copy the status file for GUI. FIX TD-504 can not change priority
	if [ -s "/tmp/greendownload/work_dir/status" ] ; then
		cp "/tmp/greendownload/work_dir/status" "/tmp/greendownload/work_dir/status.bak" 
	fi
